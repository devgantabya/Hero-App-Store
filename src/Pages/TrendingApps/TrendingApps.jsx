            ...
            <div >
                <Suspense fallback={<span>Loading...</span>} hydrateFallback={<span>Loading initial data...</span>}>
                    <div className='grid grid-cols-1 md:grid-cols-4 gap-5'>
                        {
                            data.map((singleApp) => <TrendingApp singleApp={singleApp} key={singleApp.id}></TrendingApp>)
                        }
                    </div>
                </Suspense>
            </div>
            <div className='text-center mt-15'>
                <Link to="/all-apps" className='bg-gradient-to-br from-[#632EE3] to-[#9F62F2] py-3 px-8 text-white rounded text-base font-semibold'>
                    Show All
                </Link>
            </div>
        </div>
